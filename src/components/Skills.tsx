          ...
          {Object.entries(RESUME_DATA.skills).map(([key, category]: [string, any], i) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-slate-500 text-sm mb-8 uppercase tracking-widest font-bold">/ {key}</p>
                
                <div className="flex flex-wrap gap-3 mt-auto">
                  {category.items.map((skill: string) => (
                    <span 
                      key={skill}
                      className="px-4 py-2 rounded-xl bg-white/[0.03] text-slate-300 text-sm border border-white/5 hover:bg-brand-primary/10 hover:border-brand-primary/30 hover:text-white transition-all cursor-default"
                    >
                      {skill}
                    </span>
                  ))}
